...
export class PodcastsService {
  ...
  async updatePodcast({ id, payload }: UpdatePodcastInput): Promise<CoreOutput> {
    try {
  ...
  async getEpisode({ podcastId, episodeId }: EpisodesSearchInput): Promise<GetEpisodeOutput> {
  ...
  async createEpisode({ podcastId, title, category }: CreateEpisodeInput): Promise<CreateEpisodeOutput> {
    try {
      const { podcast, ok, error } = await this.getPodcast(podcastId);
      if (!ok) {
        return { ok, error };
  ...
  async updateEpisode({ podcastId, episodeId, ...rest }: UpdateEpisodeInput): Promise<CoreOutput> {
    try {
